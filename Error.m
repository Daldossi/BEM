function [E] = Error(flag1,flag2,PT,POL,k,PWave,Omega,u,ui,X,Y,R)
...
%           oppure no ('no')
%        PT: vettore putni della mesh (sul bordo del poligono), valori complessi
...
%        u: onda 
%        ui: onda incidente di riferimento
...
err = (u_scat + PWave(Omega) - (ui+u)).*(X.^2 + Y.^2 > R);
err(isnan(err)) = 0;
E = norm(err);
end